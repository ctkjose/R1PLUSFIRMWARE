...
void gcode_M115(){
	serial_printf("ok FIRMWARE_NAME:%s\tFIRMWARE_VERSION: 1.0\tPROTOCOL_VERSION:%s\tMACHINE_TYPE:Mendel\tEXTRUDER_COUNT:%d\tUUID:%s\n", CFG_FIRMWARE_NAME,CFG_PROTOCOL_VERSION ,EXTRUDERS, CFG_MACHINE_UUID);
}
...
void gcode_initialize_methods(){
	...
	job.registerGCODE('M',25, &gcode_M25); //M25 - Pause SD print
	job.registerGCODE('M',27, &gcode_M27); //M27 - Get SD status
	job.registerGCODE('M',30, &gcode_M30); //M30 <filename> Delete File
	job.registerGCODE('M',32, &gcode_M32); //M32: Select file and start SD print
	job.registerGCODE('M',524, &gcode_M524); //M524 - Abort an SD print
